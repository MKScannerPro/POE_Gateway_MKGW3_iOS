...
@interface MKGWBXPTAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWTAdvParamsModel *dataModel;

@end

@implementation MKGWBXPTAdvParamsController

- (void)dealloc {
    NSLog(@"MKGWBXPTAdvParamsController销毁");
}
...
- (void)gw_BXPAdvParamsCell_setPressedWithSlotIndex:(NSInteger)slotIndex
    ...
    [MKGWMQTTInterface gw_configBXPTAdvParamsWithChannel:slotIndex interval:[interval integerValue] txPower:txPower bleMac:self.bleMac macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKGWTAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWTAdvParamsModel alloc] init];
    }
    return _dataModel;
}
