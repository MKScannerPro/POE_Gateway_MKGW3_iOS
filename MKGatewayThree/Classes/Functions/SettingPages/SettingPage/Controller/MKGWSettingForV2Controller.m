...
@interface MKGWSettingForV2Controller ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWSettingModel *dataModel;

@property (nonatomic, copy)NSString *localNameAsciiStr;

@end

@implementation MKGWSettingForV2Controller

- (void)dealloc {
    NSLog(@"MKGWSettingForV2Controller销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Indicator settings
        MKGWIndicatorSettingsController *vc = [[MKGWIndicatorSettingsController alloc] init];
    ...
    if (indexPath.section == 0 && indexPath.row == 4) {
        ...
        MKGWAdvBeaconV2Controller *vc = [[MKGWAdvBeaconV2Controller alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 1 && indexPath.row == 1) {
        ...
        MKGWMqttParamsListController *vc = [[MKGWMqttParamsListController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWSettingForV2Controller", @"gw_editIcon.png") forState:UIControlStateNormal];
...
- (MKGWSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWSettingModel alloc] init];
    }
    return _dataModel;
}
