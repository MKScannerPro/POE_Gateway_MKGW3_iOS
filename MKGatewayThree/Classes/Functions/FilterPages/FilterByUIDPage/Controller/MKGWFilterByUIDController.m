...
@property (nonatomic, strong)MKGWFilterByUIDModel *dataModel;

@end

@implementation MKGWFilterByUIDController

- (void)dealloc {
    NSLog(@"MKGWFilterByUIDController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWFilterByUIDController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWFilterByUIDModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWFilterByUIDModel alloc] init];
    }
    return _dataModel;
}
