...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2 && indexPath.row == 3) {
        //MK TOF
        MKGWFilterByTofModel *model = [[MKGWFilterByTofModel alloc] init];
        MKScannerFilterByTofController *vc = [[MKScannerFilterByTofController alloc] initWithProtocol:model];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 2 && indexPath.row == 4) {
        //Nano Beacon
        MKGWFilterByNanoBeaconModel *model = [[MKGWFilterByNanoBeaconModel alloc] init];
        MKScannerFilterByNanoBeaconController *vc = [[MKScannerFilterByNanoBeaconController alloc] initWithProtocol:model];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
