...
@implementation MKGWDeviceDatabaseManager

+ (BOOL)initDataBase {
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"GWDeviceDB")];
    if (![db open]) {
        return NO;
    }
    NSString *sqlCreateTable = [NSString stringWithFormat:@"create table if not exists GWDeviceTable (deviceType text,networkType text,clientID text,deviceName text,subscribedTopic text,publishedTopic text,macAddress text, lwtStatus text,lwtTopic text)"];
...
+ (void)insertDeviceList:(NSArray <MKGWDeviceModel *>*)deviceList
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"GWDeviceDB")] inDatabase:^(FMDatabase *db) {
        
        for (MKGWDeviceModel *device in deviceList) {
            BOOL exist = NO;
            FMResultSet * result = [db executeQuery:@"select * from GWDeviceTable where macAddress = ?",device.macAddress];
            while (result.next) {
...
+ (void)deleteDeviceWithMacAddress:(NSString *)macAddress
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"GWDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        BOOL result = [db executeUpdate:@"DELETE FROM GWDeviceTable WHERE macAddress = ?",macAddress];
        if (!result) {
            [self operationDeleteFailedBlock:failedBlock];
            return;
        }
...
+ (void)readLocalDeviceWithSucBlock:(void (^)(NSArray <MKGWDeviceModel *> *deviceList))sucBlock
                        failedBlock:(void (^)(NSError *error))failedBlock {
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"GWDeviceDB")];
    if (![db open]) {
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"GWDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        FMResultSet * result = [db executeQuery:@"SELECT * FROM GWDeviceTable"];
        while ([result next]) {
...
+ (void)updateLocalName:(NSString *)localName
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"GWDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        BOOL exist = NO;
        FMResultSet * result = [db executeQuery:@"select * from GWDeviceTable where macAddress = ?",macAddress];
        while (result.next) {
        ...
        //存在该设备，更新设备
        [db executeUpdate:@"UPDATE GWDeviceTable SET deviceName = ? WHERE macAddress = ?",localName,macAddress];
        if (sucBlock) {
            moko_dispatch_main_safe(^{
                sucBlock();
            });
...
+ (void)updateClientID:(NSString *)clientID
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"GWDeviceDB")] inDatabase:^(FMDatabase *db) {
        ...
        //存在该设备，更新设备
        [db executeUpdate:@"UPDATE GWDeviceTable SET clientID = ?, subscribedTopic = ? ,publishedTopic = ? , lwtStatus = ?,lwtTopic = ?,networkType = ? WHERE macAddress = ?",SafeStr(clientID),SafeStr(subscribedTopic),SafeStr(publishedTopic),(lwtStatus ? @"1" : @"0"),SafeStr(lwtTopic),SafeStr(networkType),SafeStr(macAddress)];
        if (sucBlock) {
            moko_dispatch_main_safe(^{
                sucBlock();
            });
