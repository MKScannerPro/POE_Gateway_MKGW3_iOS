//
//  MKGWFilterByBeaconController.m
...
@interface MKGWFilterByBeaconController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKGWFilterNormalTextFieldCellDelegate,
MKGWFilterBeaconCellDelegate>
...
@property (nonatomic, strong)MKGWFilterByBeaconModel *dataModel;

@end

@implementation MKGWFilterByBeaconController

- (void)dealloc {
    NSLog(@"MKGWFilterByBeaconController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MKGWFilterBeaconCell *cell = [MKGWFilterBeaconCell initCellWithTableView:tableView];
...
#pragma mark - MKGWFilterBeaconCellDelegate
- (void)mk_gw_beaconMinValueChanged:(NSString *)value index:(NSInteger)index {
    MKGWFilterBeaconCellModel *cellModel = self.section2List[index];
...
- (void)loadSection0Datas {
    ...
    cellModel.index = 0;
    if (self.pageType == mk_gw_filterByBeaconPageType_beacon) {
        cellModel.msg = @"iBeacon";
    }else if (self.pageType == mk_gw_filterByBeaconPageType_MKBeacon) {
        cellModel.msg = @"MKiBeacon";
    }else if (self.pageType == mk_gw_filterByBeaconPageType_MKBeaconAcc) {
...
- (void)loadSection1Datas {
    MKGWFilterNormalTextFieldCellModel *cellModel = [[MKGWFilterNormalTextFieldCellModel alloc] init];
    cellModel.index = 0;
    if (self.pageType == mk_gw_filterByBeaconPageType_beacon) {
...
- (void)loadSection2Datas {
    MKGWFilterBeaconCellModel *cellModel1 = [[MKGWFilterBeaconCellModel alloc] init];
    cellModel1.index = 0;
    cellModel1.minValue = self.dataModel.minMajor;
    cellModel1.maxValue = self.dataModel.maxMajor;
    [self.section2List addObject:cellModel1];
    
    MKGWFilterBeaconCellModel *cellModel2 = [[MKGWFilterBeaconCellModel alloc] init];
    cellModel2.index = 1;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWFilterByBeaconController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWFilterByBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWFilterByBeaconModel alloc] initWithPageType:self.pageType];
    }
    return _dataModel;
}
