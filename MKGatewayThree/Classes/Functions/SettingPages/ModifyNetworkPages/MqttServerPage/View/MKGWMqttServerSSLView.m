...
@implementation MKGWMqttServerSSLViewModel
@end

@interface MKGWMqttServerSSLView ()<MKGWMqttServerSSLTextFieldDelegate>
...
@property (nonatomic, strong)MKGWMqttServerSSLTextField *caFileView;

@property (nonatomic, strong)MKGWMqttServerSSLTextField *clientKeyView;

@property (nonatomic, strong)MKGWMqttServerSSLTextField *clientCertView;

@end

@implementation MKGWMqttServerSSLView
...
#pragma mark - MKGWMqttServerSSLTextFieldDelegate
/// textField内容发送改变时的回调事件
/// @param index 当前cell所在的index
/// @param value 当前textField的值
- (void)gw_modifyServerSSLTextFieldValueChanged:(NSInteger)index textValue:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_sslParams_modifyDevice_textFieldValueChanged:value:)]) {
        [self.delegate gw_mqtt_sslParams_modifyDevice_textFieldValueChanged:index value:value];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_sslParams_modifyDevice_sslStatusChanged:)]) {
        [self.delegate gw_mqtt_sslParams_modifyDevice_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(gw_mqtt_sslParams_modifyDevice_certificateChanged:)]) {
            [self.delegate gw_mqtt_sslParams_modifyDevice_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGWMqttServerSSLViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWMqttServerSSLViewModel.class]) {
        return;
    }
    ...
    MKGWMqttServerSSLTextFieldModel *caModel = [[MKGWMqttServerSSLTextFieldModel alloc] init];
    caModel.index = 0;
    ...
    MKGWMqttServerSSLTextFieldModel *clientKeyModel = [[MKGWMqttServerSSLTextFieldModel alloc] init];
    ...
    MKGWMqttServerSSLTextFieldModel *clientModel = [[MKGWMqttServerSSLTextFieldModel alloc] init];
    clientModel.index = 2;
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayThree", @"MKGWMqttServerSSLView", @"gw_switchSelectedIcon.png") : LOADICON(@"MKGatewayThree", @"MKGWMqttServerSSLView", @"gw_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayThree", @"MKGWMqttServerSSLView", @"gw_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKGWMqttServerSSLTextField *)caFileView {
    if (!_caFileView) {
        _caFileView = [[MKGWMqttServerSSLTextField alloc] init];
        _caFileView.delegate = self;
    }
    return _caFileView;
}

- (MKGWMqttServerSSLTextField *)clientKeyView {
    if (!_clientKeyView) {
        _clientKeyView = [[MKGWMqttServerSSLTextField alloc] init];
        _clientKeyView.delegate = self;
    }
    return _clientKeyView;
}

- (MKGWMqttServerSSLTextField *)clientCertView {
    if (!_clientCertView) {
        _clientCertView = [[MKGWMqttServerSSLTextField alloc] init];
