//
//  MKGWServerForDeviceController.m
...
@interface MKGWServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKGWServerConfigDeviceFooterViewDelegate,
...
@property (nonatomic, strong)MKGWServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKGWServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKGWServerConfigDeviceFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKGWServerForDeviceController

- (void)dealloc {
    NSLog(@"MKGWServerForDeviceController销毁");
}
...
#pragma mark - MKGWServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
...
- (void)gw_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)gw_selectedServerParams:(NSString *)fileName {
    [MKGWExcelDataManager parseDeviceExcel:fileName
        ...
        MKGWServerForDeviceModel *model = [MKGWServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)saveDataToDevice {
    ...
    [self.dataModel configDataWithSucBlock:^{
        ...
        [MKGWDeviceMQTTParamsModel shared].mqttConfig = YES;
        [MKGWDeviceMQTTParamsModel shared].deviceModel.clientID = self.dataModel.clientID;
        [MKGWDeviceMQTTParamsModel shared].deviceModel.deviceName = self.dataModel.deviceName;
        [MKGWDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = self.dataModel.subscribeTopic;
        [MKGWDeviceMQTTParamsModel shared].deviceModel.publishedTopic = self.dataModel.publishTopic;
        [MKGWDeviceMQTTParamsModel shared].deviceModel.macAddress = self.dataModel.macAddress;
        [MKGWDeviceMQTTParamsModel shared].deviceModel.lwtStatus = self.dataModel.lwtStatus;
        [MKGWDeviceMQTTParamsModel shared].deviceModel.lwtTopic = self.dataModel.lwtTopic;
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
        [self.view showCentralToast:error.userInfo[@"errorInfo"]];
        [MKGWDeviceMQTTParamsModel shared].deviceModel.clientID = @"";
        [MKGWDeviceMQTTParamsModel shared].deviceModel.deviceName = @"";
        [MKGWDeviceMQTTParamsModel shared].deviceModel.subscribedTopic = @"";
        [MKGWDeviceMQTTParamsModel shared].deviceModel.publishedTopic = @"";
        [MKGWDeviceMQTTParamsModel shared].deviceModel.macAddress = @"";
        [MKGWDeviceMQTTParamsModel shared].deviceModel.lwtStatus = NO;
        [MKGWDeviceMQTTParamsModel shared].deviceModel.lwtTopic = @"";
    }];
}
...
- (void)clearAllParams {
    MKGWServerForDeviceModel *emptyModel = [[MKGWServerForDeviceModel alloc] init];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWServerForDeviceController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKGWServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKGWServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKGWServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKGWServerConfigDeviceFooterViewModel alloc] init];
    }
