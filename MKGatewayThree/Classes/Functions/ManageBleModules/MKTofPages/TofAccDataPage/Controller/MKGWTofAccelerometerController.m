//
//  MKGWTofAccelerometerController.m
...
@interface MKGWTofAccelerometerController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKGWTofAccelerometerController

- (void)dealloc {
    NSLog(@"MKGWTofAccelerometerController销毁");
...
- (void)gw_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKGWMQTTInterface gw_bxpMKTofNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKGWReceiveMKTofAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
