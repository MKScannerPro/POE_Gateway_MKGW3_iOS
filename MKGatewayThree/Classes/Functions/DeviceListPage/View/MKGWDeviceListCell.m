...
@implementation MKGWDeviceListCell

+ (MKGWDeviceListCell *)initCellWithTableView:(UITableView *)tableView {
    MKGWDeviceListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGWDeviceListCellIdenty"];
    if (!cell) {
        cell = [[MKGWDeviceListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGWDeviceListCellIdenty"];
    }
    return cell;
}
...
- (void)longPressEventAction:(UILongPressGestureRecognizer *)longPress {
    if (longPress.state == UIGestureRecognizerStateBegan && [self.delegate respondsToSelector:@selector(gw_cellDeleteButtonPressed:)]) {
        [self.delegate gw_cellDeleteButtonPressed:self.indexPath.row];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGWDeviceListModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWDeviceModel.class]) {
        return;
    }
...
- (UIImageView *)wifiIcon {
    if (!_wifiIcon) {
        ...
        _wifiIcon.image = LOADICON(@"MKGatewayThree", @"MKGWDeviceListCell", @"gw_offline_wifi.png");
    }
    return _wifiIcon;
}
...
- (UIImageView *)rightIcon {
    if (!_rightIcon) {
        ...
        _rightIcon.image = LOADICON(@"MKGatewayThree", @"MKGWDeviceListCell", @"gw_goNextButton.png");
    }
    return _rightIcon;
}
