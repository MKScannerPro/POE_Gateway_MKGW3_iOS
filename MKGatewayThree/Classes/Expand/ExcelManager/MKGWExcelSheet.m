...
/**
 ...
 @return 单元格数据
 */
-(MKGWExcelCell *)getCellWithColumn:(NSString *)column row:(NSInteger )row error:(NSError **)error
{
    MKGWExcelCell *cell = nil;

    for(NSInteger i = 0 ; i < self.cellArray.count ;i++)
    {
        MKGWExcelCell *originCell = [self.cellArray objectAtIndex:i];
 ...
 */
+(NSMutableArray <MKGWExcelCell *>*)analysisSheetDataWithSheetDic:(NSDictionary *)sheetDic sharedStringsArray:(NSArray *)sharedStringsArray
{
    NSMutableArray <MKGWExcelCell *>*oneSheetAllCellArray = [self getCellArrayWithSheetDic:sheetDic sharedStringsArray:sharedStringsArray];
...
+(NSMutableArray <MKGWExcelCell *>*)getCellArrayWithSheetDic:(NSDictionary *)sheetDic sharedStringsArray:(NSArray *)sharedStringsArray
{
    NSMutableArray <MKGWExcelCell *>*oneSheetAllCellArray = nil;
                ...
                for(NSInteger i = 0 ; i < row.count ;i++)
                {
                    ...
                    if(oneRowDic && [oneRowDic isKindOfClass:[NSDictionary class]])
                    {
                        ...
                        for(NSInteger j = 0 ; j < c.count ; j ++)
                        {
                            ...
                            MKGWExcelCell *cell = [[MKGWExcelCell alloc]init];
...
+(NSString *)getMergeCellColumAndRowStrWithCell:(MKGWExcelCell *)cell mergeCellInfoArray:(NSArray *)mergeCellInfoArray
{
...
+(NSString *)getMergeStrWithMergeInfoDic:(NSDictionary *)mergeInfoDic column:(NSString *)column row:(NSInteger )row
{
    ...
    if(ref && [ref isKindOfClass:[NSString class]] && ref.length > 0)
    {
        ...
        if(array && array.count == 2)
        {
            ...
            NSInteger startRow = [[MKGWExcelCell getNumberFromStr:startStr] integerValue];
            
            NSInteger endRow = [[MKGWExcelCell getNumberFromStr:endStr] integerValue];
            
            NSString *tempStartColumnStr = [MKGWExcelCell getLetterFromStr:startStr];
            
            NSString *tempEndColumnStr = [MKGWExcelCell getLetterFromStr:endStr];
