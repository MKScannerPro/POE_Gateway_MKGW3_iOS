...
@interface MKGWNTPServerController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWNTPServerModel *dataModel;

@end

@implementation MKGWNTPServerController

- (void)dealloc {
    NSLog(@"MKGWNTPServerController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWNTPServerController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWNTPServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWNTPServerModel alloc] init];
    }
    return _dataModel;
}
