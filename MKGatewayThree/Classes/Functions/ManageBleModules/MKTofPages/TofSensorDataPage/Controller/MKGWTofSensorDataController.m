...
@interface MKGWTofSensorDataController ()<MFMailComposeViewControllerDelegate,
...
@implementation MKGWTofSensorDataController

- (void)dealloc {
    NSLog(@"MKGWTofSensorDataController销毁");
...
- (void)gw_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKGWMQTTInterface gw_bxpMKTofNotifySensorDataWithBleMac:self.bleMac notify:isOn macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
