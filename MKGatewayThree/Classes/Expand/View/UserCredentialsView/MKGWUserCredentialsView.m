...
@implementation MKGWUserCredentialsViewModel
@end

@interface MKGWUserCredentialsView ()
...
@implementation MKGWUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKGWUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(gw_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate gw_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(gw_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate gw_mqtt_userCredentials_passwordChanged:text];
            }
        };
