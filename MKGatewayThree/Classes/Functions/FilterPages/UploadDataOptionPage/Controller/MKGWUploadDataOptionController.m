//
//  MKGWUploadDataOptionController.m
...
@interface MKGWUploadDataOptionController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWUploadDataOptionModel *dataModel;

@end

@implementation MKGWUploadDataOptionController

- (void)dealloc {
    NSLog(@"MKGWUploadDataOptionController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWUploadDataOptionController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWUploadDataOptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWUploadDataOptionModel alloc] init];
    }
    return _dataModel;
}
