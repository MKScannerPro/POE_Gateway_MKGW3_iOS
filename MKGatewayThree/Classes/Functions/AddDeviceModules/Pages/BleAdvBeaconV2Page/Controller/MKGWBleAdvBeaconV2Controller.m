//
//  MKGWBleAdvBeaconV2Controller.m
...
@interface MKGWBleAdvBeaconV2Controller ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWBleAdvBeaconV2Model *dataModel;

@end

@implementation MKGWBleAdvBeaconV2Controller

- (void)dealloc {
    NSLog(@"MKGWBleAdvBeaconV2Controller销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWBleAdvBeaconV2Controller", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWBleAdvBeaconV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWBleAdvBeaconV2Model alloc] init];
    }
    return _dataModel;
}
