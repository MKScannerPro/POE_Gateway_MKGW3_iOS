//
//  MKGWDuplicateDataFilterController.m
...
@interface MKGWDuplicateDataFilterController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWDuplicateDataFilterModel *dataModel;

@end

@implementation MKGWDuplicateDataFilterController

- (void)dealloc {
    NSLog(@"MKGWDuplicateDataFilterController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWDuplicateDataFilterController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWDuplicateDataFilterModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWDuplicateDataFilterModel alloc] init];
    }
    return _dataModel;
}
