...
#import "MKGWExcelWookbook.h"

static NSString *const defaultKeyValueString = @"value:";

@implementation MKGWExcelDataManager

+ (void)exportAppExcel:(id <MKGWExcelAppProtocol>)protocol
              sucBlock:(void(^)(void))sucBlock
           failedBlock:(void(^)(NSError *error))failedBlock {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKGWExcelAppProtocol)]) {
...
+ (void)parseAppExcel:(NSString *)excelName
    ...
    MKGWExcelWookbook *workbook = [[MKGWExcelWookbook alloc] initWithExcelFilePathUrl:excelUrl];
    if (!workbook || workbook.sheetArray.count == 0) {
        [self operationFailedBlockWithMsg:@"Load Excel Data Failed" block:failedBlock];
        return;
    }
    MKGWExcelSheet *sheet = workbook.sheetArray.firstObject;
    if (![sheet isKindOfClass:MKGWExcelSheet.class]) {
        [self operationFailedBlockWithMsg:@"Load Excel Data Failed" block:failedBlock];
        return;
    }
    ...
    //根据横竖坐标，获取单元格
    //Host
    MKGWExcelCell *hostCell = [sheet getCellWithColumn:@"B" row:2 error:nil];
    ...
    //Port
    MKGWExcelCell *portCell = [sheet getCellWithColumn:@"B" row:3 error:nil];
    ...
    //Client id
    MKGWExcelCell *clientIDCell = [sheet getCellWithColumn:@"B" row:4 error:nil];
    ...
    //Subscribe Topic
    MKGWExcelCell *subTopicCell = [sheet getCellWithColumn:@"B" row:5 error:nil];
    ...
    //Publish Topic
    MKGWExcelCell *pubTopicCell = [sheet getCellWithColumn:@"B" row:6 error:nil];
    ...
    //Clean Session
    MKGWExcelCell *cleanSessionCell = [sheet getCellWithColumn:@"B" row:7 error:nil];
    ...
    //Qos
    MKGWExcelCell *qosCell = [sheet getCellWithColumn:@"B" row:8 error:nil];
    ...
    //Keep Alive
    MKGWExcelCell *keepAliveCell = [sheet getCellWithColumn:@"B" row:9 error:nil];
...
//    }
    ...
    //MQTT Username
    MKGWExcelCell *usernameCell = [sheet getCellWithColumn:@"B" row:10 error:nil];
    ...
    //MQTT Password
    MKGWExcelCell *passwordCell = [sheet getCellWithColumn:@"B" row:11 error:nil];
    ...
    //SSL/TLS
    MKGWExcelCell *sslCell = [sheet getCellWithColumn:@"B" row:12 error:nil];
    ...
    //Certificate type
    MKGWExcelCell *certificateCell = [sheet getCellWithColumn:@"B" row:13 error:nil];
...
+ (void)exportDeviceExcel:(id <MKGWExcelDeviceProtocol>)protocol
                 sucBlock:(void(^)(void))sucBlock
              failedBlock:(void(^)(NSError *error))failedBlock {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKGWExcelDeviceProtocol)]) {
        [self operationFailedBlockWithMsg:@"Params Error" block:failedBlock];
        return;
    }
...
//    }
    ...
    [dic setObject:@(certValue) forKey:@"certificate"];
    //LWT
    MKGWExcelCell *lwtCell = [sheet getCellWithColumn:@"B" row:14 error:nil];
    ...
    //LWT Retain
    MKGWExcelCell *lwtRetainCell = [sheet getCellWithColumn:@"B" row:15 error:nil];
    ...
    //LWT Qos
    MKGWExcelCell *lwtQosCell = [sheet getCellWithColumn:@"B" row:16 error:nil];
    ...
    //LWT Topic
    MKGWExcelCell *lwtTopicCell = [sheet getCellWithColumn:@"B" row:17 error:nil];
    ...
    //LWT Payload
    MKGWExcelCell *lwtPayloadCell = [sheet getCellWithColumn:@"B" row:18 error:nil];
...
+ (BOOL)checkExcelAppProtocol:(id <MKGWExcelAppProtocol>)protocol {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKGWExcelAppProtocol)]) {
        return NO;
    }
...
+ (BOOL)checkExcelDeviceProtocol:(id <MKGWExcelDeviceProtocol>)protocol {
    if (!protocol || ![protocol conformsToProtocol:@protocol(MKGWExcelDeviceProtocol)]) {
        return NO;
    }
    if (!ValidStr(protocol.host) || protocol.host.length > 64 || ![protocol.host isAsciiString]) {
