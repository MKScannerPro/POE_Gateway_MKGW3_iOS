...
#import "ZipArchive.h"
#import "MKGWXMLReader.h"

@interface MKGWExcelWookbook()
...
@implementation MKGWExcelWookbook
                ...
                if(writeSuccess)
                {
                    ...
                    if(uncompressionSuccess)
                    {
                        ...
                        for(NSInteger i = 0 ; i < array.count ; i ++)
                        {
                            ...
                            MKGWExcelSheet *sheet = [[MKGWExcelSheet alloc]init];
                            
                            sheet.sheetId = [[sheetDic objectForKey:@"sheetId"] integerValue];
                            
                            sheet.cellArray = [MKGWExcelSheet analysisSheetDataWithSheetDic:sheetDic sharedStringsArray:stringValueArray];
...
-(NSDictionary *)getSharedStringsWithPath:(NSString *)path
{
    ...
    if(xmlData && xmlData.length > 0)
    {
        ...
        xmlDic = [MKGWXMLReader dictionaryForXMLData:xmlData error:&error];
    }
    return xmlDic;
}
    ...
    while (fileName = [dirEnum nextObject]) {
        ...
        if ([fileName hasSuffix:@".xml"]) {
            ...
            if(xmlData && xmlData.length > 0)
            {
                ...
                NSInteger sheetId = [[MKGWExcelCell getNumberFromStr:fileName] integerValue];
                
                NSMutableDictionary *sheetDic = [[NSMutableDictionary alloc]init];
                
                NSDictionary *xmlDic = [MKGWXMLReader dictionaryForXMLData:xmlData error:&error];
...
-(NSArray *)getSheedNameInfoArrayWithPath:(NSString *)path
{
    ...
    NSDictionary *sheetNameInfoDic = [MKGWXMLReader dictionaryForXMLData:xmlData error:&error];
