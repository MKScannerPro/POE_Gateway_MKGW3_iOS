...
@implementation MKGWNearbyWifiCellModel
@end

@interface MKGWNearbyWifiCell ()
...
@implementation MKGWNearbyWifiCell


+ (MKGWNearbyWifiCell *)initCellWithTableView:(UITableView *)tableView {
    MKGWNearbyWifiCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGWNearbyWifiCellIdenty"];
    if (!cell) {
        cell = [[MKGWNearbyWifiCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGWNearbyWifiCellIdenty"];
    }
    return cell;
}
...
#pragma mark - setter
- (void)setDataModel:(MKGWNearbyWifiCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWNearbyWifiCellModel.class]) {
        return;
    }
    //顶部
