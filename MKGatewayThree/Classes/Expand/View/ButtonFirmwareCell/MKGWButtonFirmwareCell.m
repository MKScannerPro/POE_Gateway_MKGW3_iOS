...
@implementation MKGWButtonFirmwareCellModel
@end

@interface MKGWButtonFirmwareCell ()
...
@implementation MKGWButtonFirmwareCell

+ (MKGWButtonFirmwareCell *)initCellWithTableView:(UITableView *)tableView {
    MKGWButtonFirmwareCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGWButtonFirmwareCellIdenty"];
    if (!cell) {
        cell = [[MKGWButtonFirmwareCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGWButtonFirmwareCellIdenty"];
    }
    return cell;
}
...
- (void)rightButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gw_buttonFirmwareCell_buttonAction:)]) {
        [self.delegate gw_buttonFirmwareCell_buttonAction:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGWButtonFirmwareCellModel *)dataModel {
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWButtonFirmwareCellModel.class]) {
        return;
    }
