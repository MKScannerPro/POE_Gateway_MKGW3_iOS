...
@implementation MKGWBXPSController

- (void)dealloc {
    NSLog(@"MKGWBXPSController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Real time T&H data
        MKGWBXPSRealTimeTHDataController *vc = [[MKGWBXPSRealTimeTHDataController alloc] init];
    ...
    if (indexPath.section == 3 && indexPath.row == 1) {
        //Historical T&H data
        MKGWBXPSHistoricalTHDataController *vc = [[MKGWBXPSHistoricalTHDataController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 2) {
        //Accelerometer data
        MKGWBXPSAccelerometerController *vc = [[MKGWBXPSAccelerometerController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 3) {
        //Hall sensor data
        MKGWBXPSHallCountController *vc = [[MKGWBXPSHallCountController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 4) {
        ...
        MKGWBXPSTHDataSampleRateController *vc = [[MKGWBXPSTHDataSampleRateController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    ...
    if (indexPath.section == 3 && indexPath.row == 6) {
        //Remote reminder
        MKGWBXPSRemoteReminderController *vc = [[MKGWBXPSRemoteReminderController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)readDatasFromDevice {
    ...
    [MKGWMQTTInterface gw_readBXPSConnectedStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)powerOffCmdToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGWMQTTInterface gw_bxpBXPSPowerOffWithBleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.navigationController popViewControllerAnimated:YES];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
