//
//  MKGWManageBleDevicesSearchButton.m
...
@implementation MKGWManageBleDevicesSearchButtonModel
@end

@interface MKGWManageBleDevicesSearchButton ()
...
@implementation MKGWManageBleDevicesSearchButton
...
- (void)clearButtonPressed{
    ...
    if ([self.delegate respondsToSelector:@selector(gw_scanSearchButtonClearMethod)]) {
        [self.delegate gw_scanSearchButtonClearMethod];
    }
}

- (void)searchButtonPressed{
    if ([self.delegate respondsToSelector:@selector(gw_scanSearchButtonMethod)]) {
        [self.delegate gw_scanSearchButtonMethod];
    }
}

#pragma mark - setting
- (void)setDataModel:(MKGWManageBleDevicesSearchButtonModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWManageBleDevicesSearchButtonModel.class]) {
        return;
    }
...
- (UIImageView *)searchIcon{
    if (!_searchIcon) {
        ...
        _searchIcon.image = LOADICON(@"MKGatewayThree", @"MKGWManageBleDevicesSearchButton", @"gw_searchGrayIcon.png");
    }
    return _searchIcon;
}
...
- (UIButton *)clearButton{
    if (!_clearButton) {
        ...
        [_clearButton setImage:LOADICON(@"MKGatewayThree", @"MKGWManageBleDevicesSearchButton", @"gw_clearButtonIcon.png") forState:UIControlStateNormal];
