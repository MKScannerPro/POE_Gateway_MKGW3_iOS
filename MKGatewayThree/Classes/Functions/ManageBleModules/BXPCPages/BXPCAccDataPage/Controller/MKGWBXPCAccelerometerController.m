//
//  MKGWBXPCAccelerometerController.m
...
@interface MKGWBXPCAccelerometerController ()<MFMailComposeViewControllerDelegate,
MKGWBXPButtonAccHeaderViewDelegate>

@property (nonatomic, strong)MKGWBXPButtonAccHeaderView *headerView;
...
@implementation MKGWBXPCAccelerometerController

- (void)dealloc {
    NSLog(@"MKGWBXPCAccelerometerController销毁");
...
- (void)gw_bxpButtonAccHeaderView_syncButtonPressed:(BOOL)isOn {
    ...
    [MKGWMQTTInterface gw_bxpBXPCNotifyAccDataWithBleMac:self.bleMac notify:isOn macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        ...
        if (isOn) {
            [[NSNotificationCenter defaultCenter] addObserver:self
                                                     selector:@selector(receiveAccDatas:)
                                                         name:MKGWReceiveBXPCAccDataNotification
                                                       object:nil];
        }else {
            [[NSNotificationCenter defaultCenter] removeObserver:self];
        }
...
#pragma mark - getter
- (MKGWBXPButtonAccHeaderView *)headerView {
    if (!_headerView) {
        _headerView = [[MKGWBXPButtonAccHeaderView alloc] init];
