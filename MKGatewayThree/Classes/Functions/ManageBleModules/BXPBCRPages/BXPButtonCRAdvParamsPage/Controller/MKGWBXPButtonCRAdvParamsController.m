//
//  MKGWBXPButtonCRAdvParamsController.m
...
@interface MKGWBXPButtonCRAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWBXPButtonCRAdvParamsModel *dataModel;

@end

@implementation MKGWBXPButtonCRAdvParamsController

- (void)dealloc {
    NSLog(@"MKGWBXPButtonCRAdvParamsController销毁");
}
...
- (void)gw_advNormalCell_setPressed:(NSInteger)index
    ...
    [MKGWMQTTInterface gw_bxpBtnCRConfigAdvParamsWithParams:param bleMac:self.bleMac macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (UITableViewCell *)loadTableCell:(NSIndexPath *)indexPath {
    ...
    if (!ValidDict(advParams)) {
        return [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGWBXPButtonCRAdvParamsControllerCell"];
    }
...
- (MKGWBXPButtonCRAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWBXPButtonCRAdvParamsModel alloc] init];
    }
    return _dataModel;
}
