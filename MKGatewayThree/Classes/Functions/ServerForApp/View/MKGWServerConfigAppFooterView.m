//
//  MKGWServerConfigAppFooterView.m
...
@implementation MKGWServerConfigAppFooterViewModel
@end
...
@interface MKGWServerConfigAppFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKGWMQTTSSLForAppViewDelegate>
...
@property (nonatomic, strong)MKGWMQTTSSLForAppView *sslView;
...
@implementation MKGWServerConfigAppFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate gw_mqtt_serverForApp_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_qosChanged:)]) {
        [self.delegate gw_mqtt_serverForApp_qosChanged:qos];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate gw_mqtt_serverForApp_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate gw_mqtt_serverForApp_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_textFieldValueChanged:textID:)]) {
        [self.delegate gw_mqtt_serverForApp_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKGWMQTTSSLForAppViewDelegate
- (void)gw_mqtt_sslParams_app_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_switchStatusChanged:statusID:)]) {
        [self.delegate gw_mqtt_serverForApp_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)gw_mqtt_sslParams_app_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_certificateChanged:)]) {
        [self.delegate gw_mqtt_serverForApp_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)gw_mqtt_sslParams_app_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate gw_mqtt_serverForApp_fileButtonPressed:0];
    }
}
...
- (void)gw_mqtt_sslParams_app_clientFilePressed {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_fileButtonPressed:)]) {
        [self.delegate gw_mqtt_serverForApp_fileButtonPressed:1];
    }
}
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_bottomButtonPressed:)]) {
        [self.delegate gw_mqtt_serverForApp_bottomButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_bottomButtonPressed:)]) {
        [self.delegate gw_mqtt_serverForApp_bottomButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForApp_bottomButtonPressed:)]) {
        [self.delegate gw_mqtt_serverForApp_bottomButtonPressed:1];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKGWServerConfigAppFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWServerConfigAppFooterViewModel.class]) {
        return;
    }
    ...
    MKGWMQTTSSLForAppViewModel *sslModel = [[MKGWMQTTSSLForAppViewModel alloc] init];
...
- (MKGWMQTTSSLForAppView *)sslView {
    if (!_sslView) {
        _sslView = [[MKGWMQTTSSLForAppView alloc] init];
