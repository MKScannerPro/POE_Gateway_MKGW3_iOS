...
static MKGWMQTTServerManager *manager = nil;
static dispatch_once_t onceToken;

@interface NSObject (MKGWMQTTServerManager)

@end

@implementation NSObject (MKGWMQTTServerManager)
...
@interface MKGWMQTTServerManager ()

@property (nonatomic, assign)MKGWMQTTSessionManagerState state;

@property (nonatomic, strong)MKGWServerParamsModel *paramsModel;

@property (nonatomic, strong)NSMutableArray <id <MKGWServerManagerProtocol>>*managerList;

@property (nonatomic, strong)NSMutableDictionary *subscriptions;

@end

@implementation MKGWMQTTServerManager
...
+ (MKGWMQTTServerManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKGWMQTTServerManager new];
        }
    });
    return manager;
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager
    ...
    @synchronized (self.managerList) {
        for (id <MKGWServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(gw_didReceiveMessage:onTopic:)]) {
                [protocol gw_didReceiveMessage:dataDic onTopic:topic];
            }
        }
    }
}
...
- (void)sessionManager:(MQTTSessionManager *)sessionManager didChangeState:(MKMQTTSessionManagerState)newState {
    ...
    @synchronized (self.managerList) {
        for (id <MKGWServerManagerProtocol>protocol in self.managerList) {
            if ([protocol respondsToSelector:@selector(gw_didChangeState:)]) {
                [protocol gw_didChangeState:self.state];
            }
        }
    }
}
...
- (void)networkStateChanged{
    ...
    if (![[MKNetworkManager sharedInstance] currentNetworkAvailable]) {
        ...
        [self sessionManager:manager didChangeState:MKGWMQTTSessionManagerStateStarting];
        return;
    }
...
- (void)loadDataManager:(nonnull id <MKGWServerManagerProtocol>)dataManager {
    @synchronized (self.managerList) {
        if (dataManager
            && [dataManager conformsToProtocol:@protocol(MKGWServerManagerProtocol)]
...
- (void)loadLogFile {
    ...
    NSString *recordDateString = [[NSUserDefaults standardUserDefaults] objectForKey:@"mk_gw_mqtt_recordDate"];
    if (!ValidStr(recordDateString)) {
        //如果第一次存，则把当天的日期写入
        [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_gw_mqtt_recordDate"];
        return;
    }
    //写入当天日期，用来做比较
    [[NSUserDefaults standardUserDefaults] setObject:[dateFormatter stringFromDate:[NSDate date]] forKey:@"mk_gw_mqtt_recordDate"];
...
#pragma mark - getter
- (MKGWServerParamsModel *)paramsModel {
    if (!_paramsModel) {
        _paramsModel = [[MKGWServerParamsModel alloc] init];
    }
    return _paramsModel;
}

- (NSMutableArray<id<MKGWServerManagerProtocol>> *)managerList {
    if (!_managerList) {
