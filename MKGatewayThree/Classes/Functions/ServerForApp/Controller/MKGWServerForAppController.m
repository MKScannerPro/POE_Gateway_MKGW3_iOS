...
@implementation MKGWServerForAppController

- (void)dealloc {
    NSLog(@"MKGWServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKGWMQTTDataManager shared] saveServerParams:self.dataModel];
    ...
    [[MKGWMQTTDataManager shared] connect];
}
...
- (void)serverManagerStateChanged {
    ...
    if ([MKGWMQTTDataManager shared].state == MKGWMQTTSessionManagerStateConnected) {
        ...
        [[NSNotificationCenter defaultCenter] removeObserver:self
                                                        name:MKGWMQTTSessionManagerStateChangedNotification
                                                      object:nil];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_gw_needReloadTopicsNotification"
                                                            object:nil];
...
- (void)connectFailed {
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                    name:MKGWMQTTSessionManagerStateChangedNotification
                                                  object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"MKGWMQTTServerConnectFailedNotification" object:nil];
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWServerForAppController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWServerForAppModel alloc] init];
    }
    return _dataModel;
}
