...
@interface MKGWFilterByPirController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWFilterByPirModel *dataModel;

@end

@implementation MKGWFilterByPirController

- (void)dealloc {
    NSLog(@"MKGWFilterByPirController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWFilterByPirController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWFilterByPirModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWFilterByPirModel alloc] init];
    }
    return _dataModel;
}
