...
@interface MKGWFilterByTofController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWFilterByTofModel *dataModel;

@end

@implementation MKGWFilterByTofController

- (void)dealloc {
    NSLog(@"MKGWFilterByTofController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWFilterByTofController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWFilterByTofModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWFilterByTofModel alloc] init];
    }
    return _dataModel;
}
