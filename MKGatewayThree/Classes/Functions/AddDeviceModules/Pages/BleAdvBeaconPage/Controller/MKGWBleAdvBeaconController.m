...
@interface MKGWBleAdvBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWBleAdvBeaconModel *dataModel;

@end

@implementation MKGWBleAdvBeaconController

- (void)dealloc {
    NSLog(@"MKGWBleAdvBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWBleAdvBeaconController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWBleAdvBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWBleAdvBeaconModel alloc] init];
    }
    return _dataModel;
}
