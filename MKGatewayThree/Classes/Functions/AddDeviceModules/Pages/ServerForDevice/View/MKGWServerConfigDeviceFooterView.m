//
//  MKGWServerConfigDeviceFooterView.m
...
@implementation MKGWServerConfigDeviceFooterViewModel
@end
...
@interface MKGWServerConfigDeviceFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKGWMQTTSSLForDeviceViewDelegate,
MKGWServerConfigDeviceSettingViewDelegate,
MKGWMQTTLWTForDeviceViewDelegate>
...
@property (nonatomic, strong)MKGWMQTTSSLForDeviceView *sslView;

@property (nonatomic, strong)MKGWMQTTLWTForDeviceView *lwtView;

@property (nonatomic, strong)MKGWServerConfigDeviceSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKGWServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate gw_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate gw_mqtt_serverForDevice_qosChanged:qos qosID:0];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gw_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)mk_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gw_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)mk_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gw_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKGWMQTTSSLForDeviceViewDelegate
- (void)gw_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate gw_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)gw_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate gw_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)gw_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate gw_mqtt_serverForDevice_fileButtonPressed:0];
    }
}
...
- (void)gw_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate gw_mqtt_serverForDevice_fileButtonPressed:1];
    }
}
...
- (void)gw_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate gw_mqtt_serverForDevice_fileButtonPressed:2];
    }
}

#pragma mark - MKGWServerConfigDeviceSettingViewDelegate

/// 底部按钮
/// @param index 0:Export Demo File   1:Import Config File
- (void)gw_mqtt_deviecSetting_fileButtonPressed:(NSInteger)index {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_bottomButtonPressed:)]) {
        [self.delegate gw_mqtt_serverForDevice_bottomButtonPressed:index];
    }
}

#pragma mark - MKGWMQTTLWTForDeviceViewDelegate
- (void)gw_lwt_statusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate gw_mqtt_serverForDevice_switchStatusChanged:isOn statusID:2];
    }
}

- (void)gw_lwt_retainChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate gw_mqtt_serverForDevice_switchStatusChanged:isOn statusID:3];
    }
}

- (void)gw_lwt_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate gw_mqtt_serverForDevice_qosChanged:qos qosID:1];
    }
}

- (void)gw_lwt_topicChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gw_mqtt_serverForDevice_textFieldValueChanged:text textID:5];
    }
}

- (void)gw_lwt_payloadChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gw_mqtt_serverForDevice_textFieldValueChanged:text textID:6];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKGWServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKGWMQTTSSLForDeviceViewModel *sslModel = [[MKGWMQTTSSLForDeviceViewModel alloc] init];
    ...
    MKGWMQTTLWTForDeviceViewModel *lwtModel = [[MKGWMQTTLWTForDeviceViewModel alloc] init];
...
- (MKGWMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKGWMQTTSSLForDeviceView alloc] init];
...
- (MKGWMQTTLWTForDeviceView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKGWMQTTLWTForDeviceView alloc] init];
...
- (MKGWServerConfigDeviceSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKGWServerConfigDeviceSettingView alloc] init];
