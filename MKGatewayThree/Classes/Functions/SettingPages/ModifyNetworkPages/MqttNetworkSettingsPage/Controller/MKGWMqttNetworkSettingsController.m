//
//  MKGWMqttNetworkSettingsController.m
...
@interface MKGWMqttNetworkSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWMqttNetworkSettingsModel *dataModel;

@end

@implementation MKGWMqttNetworkSettingsController

- (void)dealloc {
    NSLog(@"MKGWMqttNetworkSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWMqttNetworkSettingsController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWMqttNetworkSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWMqttNetworkSettingsModel alloc] init];
    }
    return _dataModel;
}
