...
@interface MKGWBXPDController ()<UITableViewDelegate,
...
@implementation MKGWBXPDController

- (void)dealloc {
    NSLog(@"MKGWBXPDController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Accelerometer data
        MKGWBXPDAccelerometerController *vc = [[MKGWBXPDAccelerometerController alloc] init];
    ...
    if (indexPath.section == 3 && indexPath.row == 1) {
        ...
        MKGWBXPDAccParamsController *vc = [[MKGWBXPDAccParamsController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 3 && indexPath.row == 2) {
        ...
        MKGWBXPDAdvParamsController *vc = [[MKGWBXPDAdvParamsController alloc] init];
        vc.bleMac = self.deviceBleInfo[@"data"][@"mac"];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)readDatasFromDevice {
    ...
    [MKGWMQTTInterface gw_readBXPDConnectedStatusWithBleMacAddress:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (void)powerOffCmdToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKGWMQTTInterface gw_bxpBXPDPowerOffWithBleMac:self.deviceBleInfo[@"data"][@"mac"] macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        [[MKHudManager share] hide];
        [self.navigationController popViewControllerAnimated:YES];
    } failedBlock:^(NSError * _Nonnull error) {
        [[MKHudManager share] hide];
