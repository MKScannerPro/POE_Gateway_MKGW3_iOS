...
@interface MKGWNearbyWifiController ()<UITableViewDelegate,
UITableViewDataSource,
mk_gw_centralManagerScanWifiDelegate>
...
@implementation MKGWNearbyWifiController

- (void)dealloc {
    NSLog(@"MKGWNearbyWifiController销毁");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    MKGWNearbyWifiCellModel *cellModel = self.dataList[indexPath.row];
    if ([self.delegate respondsToSelector:@selector(gw_nearbyWifiController_selectedWifi:)]) {
        [self.delegate gw_nearbyWifiController_selectedWifi:cellModel.ssid];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKGWNearbyWifiCell *cell = [MKGWNearbyWifiCell initCellWithTableView:tableView];
...
- (void)parseContentList {
    ...
    while (index < length) {
        ...
        // 创建模型对象
        MKGWNearbyWifiCellModel *model = [[MKGWNearbyWifiCellModel alloc] init];
        model.bssid = bssid;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWNearbyWifiController", @"gw_refreshWifiListIcon.png") forState:UIControlStateNormal];
