...
@implementation MKGWManageBleDevicesCellModel
@end

@interface MKGWManageBleDevicesCell ()
...
@implementation MKGWManageBleDevicesCell

+ (MKGWManageBleDevicesCell *)initCellWithTableView:(UITableView *)tableView {
    MKGWManageBleDevicesCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGWManageBleDevicesCellIdenty"];
    if (!cell) {
        cell = [[MKGWManageBleDevicesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGWManageBleDevicesCellIdenty"];
    }
    return cell;
}
...
- (void)connectButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gw_manageBleDevicesCellConnectButtonPressed:typeCode:)]) {
        [self.delegate gw_manageBleDevicesCellConnectButtonPressed:SafeStr(self.dataModel.macAddress)
                                                          typeCode:self.dataModel.typeCode];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGWManageBleDevicesCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWManageBleDevicesCellModel.class]) {
        return;
    }
