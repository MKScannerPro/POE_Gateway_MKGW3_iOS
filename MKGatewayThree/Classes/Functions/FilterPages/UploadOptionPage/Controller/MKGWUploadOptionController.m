...
@implementation MKGWUploadOptionController
...
- (void)filterByRawData {
    MKGWFilterByRawDataController *vc = [[MKGWFilterByRawDataController alloc] init];
    [self.navigationController pushViewController:vc animated:YES];
}
...
- (MKGWUploadOptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWUploadOptionModel alloc] init];
    }
    return _dataModel;
}
