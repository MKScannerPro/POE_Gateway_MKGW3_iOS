//
//  MKGWBleNTPTimezoneController.m
...
@interface MKGWBleNTPTimezoneController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWBleNTPTimezoneModel *dataModel;

@property (nonatomic, strong)NSMutableArray *timezoneList;

@end

@implementation MKGWBleNTPTimezoneController

- (void)dealloc {
    NSLog(@"MKGWBleNTPTimezoneController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWBleNTPTimezoneController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWBleNTPTimezoneModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWBleNTPTimezoneModel alloc] init];
    }
    return _dataModel;
}
