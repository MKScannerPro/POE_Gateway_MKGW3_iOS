...
@interface MKGWBXPDAccParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWBXPDAccParamsModel *dataModel;

@end

@implementation MKGWBXPDAccParamsController

- (void)dealloc {
    NSLog(@"MKGWBXPDAccParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWBXPDAccParamsController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWBXPDAccParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWBXPDAccParamsModel alloc] init];
    }
    return _dataModel;
}
