//
//  MKGWMqttServerConfigFooterView.m
...
@implementation MKGWMqttServerConfigFooterViewModel
@end
...
@interface MKGWMqttServerConfigFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKMQTTUserCredentialsViewDelegate,
MKGWMqttServerSSLViewDelegate,
MKGWMqttServerSettingViewDelegate,
MKGWMqttServerLwtViewDelegate>
...
@property (nonatomic, strong)MKGWMqttServerSSLView *sslView;

@property (nonatomic, strong)MKGWMqttServerLwtView *lwtView;

@property (nonatomic, strong)MKGWMqttServerSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKGWMqttServerConfigFooterView
...
#pragma mark - MKGWMqttServerSSLViewDelegate
...
- (void)gw_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    ...
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate gw_mqtt_serverForDevice_textFieldValueChanged:value textID:tempIndex];
    }
}

#pragma mark - MKGWMqttServerSettingViewDelegate
...
#pragma mark - MKGWMqttServerLwtViewDelegate
...
#pragma mark - setter
- (void)setDataModel:(MKGWMqttServerConfigFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWMqttServerConfigFooterViewModel.class]) {
        return;
    }
    ...
    MKGWMqttServerSSLViewModel *sslModel = [[MKGWMqttServerSSLViewModel alloc] init];
    ...
    MKGWMqttServerLwtViewModel *lwtModel = [[MKGWMqttServerLwtViewModel alloc] init];
...
- (MKGWMqttServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKGWMqttServerSSLView alloc] init];
...
- (MKGWMqttServerLwtView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKGWMqttServerLwtView alloc] init];
...
- (MKGWMqttServerSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKGWMqttServerSettingView alloc] init];
