...
#import "MKGWFilterByRawDataCell.h"
#import "MKGWFilterEditSectionHeaderView.h"

#import "MKGWFilterByOtherModel.h"

@interface MKGWFilterByOtherController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKGWFilterEditSectionHeaderViewDelegate,
MKGWFilterByRawDataCellDelegate,
...
@property (nonatomic, strong)MKGWFilterByOtherModel *dataModel;

@end

@implementation MKGWFilterByOtherController

- (void)dealloc {
    NSLog(@"MKGWFilterByOtherController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKGWFilterEditSectionHeaderView *headerView = [MKGWFilterEditSectionHeaderView initHeaderViewWithTableView:tableView];
        MKGWFilterEditSectionHeaderViewModel *model = [[MKGWFilterEditSectionHeaderViewModel alloc] init];
        model.index = 0;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKGWFilterByRawDataCell *cell = [MKGWFilterByRawDataCell initCellWithTableView:tableView];
...
#pragma mark - MKGWFilterEditSectionHeaderViewDelegate
/// 加号点击事件
/// @param index 所在index
- (void)mk_gw_filterEditSectionHeaderView_addButtonPressed:(NSInteger)index {
    ...
    MKGWFilterByRawDataCellModel *cellModel = [[MKGWFilterByRawDataCellModel alloc] init];
    if (cellModelIndex == 0) {
        //Condition A
...
#pragma mark - MKGWFilterByRawDataCellDelegate
/// 输入框内容发生改变
...
- (void)gw_rawFilterDataChanged:(mk_gw_filterRawAdvDataTextType)textType
    ...
    MKGWFilterByRawDataCellModel *cellModel = self.section1List[index];
    if (textType == mk_gw_filterRawAdvDataTextTypeDataType) {
        //过滤类型输入框内容发生改变
        cellModel.dataType = textValue;
        return;
    }
    if (textType == mk_gw_filterRawAdvDataTextTypeMinIndex) {
        //开始过滤的Byte索引输入框发生改变
        cellModel.minIndex = textValue;
        return;
    }
    if (textType == mk_gw_filterRawAdvDataTextTypeMaxIndex) {
        //截止过滤的Byte索引输入框发生改变
        cellModel.maxIndex = textValue;
        return;
    }
    if (textType == mk_gw_filterRawAdvDataTextTypeRawDataType) {
        //过滤内容输入框发生改变
...
- (void)configDataToDevice {
    ...
    for (NSInteger i = 0; i < self.section1List.count; i ++) {
        MKGWFilterByRawDataCellModel *cellModel = self.section1List[i];
        ...
        MKGWFilterRawAdvDataModel *model = [[MKGWFilterRawAdvDataModel alloc] init];
...
- (mk_gw_filterByOther)loadCurrentRelationship {
    MKTextButtonCellModel *cellModel = self.section2List[0];
    if (self.section1List.count == 1) {
        //@[@"A"];
        return mk_gw_filterByOther_A;
    }
    if (self.section1List.count == 2) {
        //@[@"A & B",@"A | B"]
        if (cellModel.dataListIndex == 0) {
            return mk_gw_filterByOther_AB;
        }
        return mk_gw_filterByOther_AOrB;
    }
    if (self.section1List.count == 3) {
        //@[@"A & B & C",@"(A & B) | C",@"A | B | C"]
        if (cellModel.dataListIndex == 0) {
            return mk_gw_filterByOther_ABC;
        }
        if (cellModel.dataListIndex == 1) {
            return mk_gw_filterByOther_ABOrC;
        }
        if (cellModel.dataListIndex == 2) {
            return mk_gw_filterByOther_AOrBOrC;
        }
    }
    return mk_gw_filterByOther_AOrBOrC;
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWFilterByOtherController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWFilterByOtherModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWFilterByOtherModel alloc] init];
    }
    return _dataModel;
}
