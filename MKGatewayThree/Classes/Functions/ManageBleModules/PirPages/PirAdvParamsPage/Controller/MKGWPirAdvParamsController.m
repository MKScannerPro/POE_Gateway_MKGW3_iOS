...
@interface MKGWPirAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWPirAdvParamsModel *dataModel;

@end

@implementation MKGWPirAdvParamsController

- (void)dealloc {
    NSLog(@"MKGWPirAdvParamsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWPirAdvParamsController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWPirAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWPirAdvParamsModel alloc] init];
    }
    return _dataModel;
}
