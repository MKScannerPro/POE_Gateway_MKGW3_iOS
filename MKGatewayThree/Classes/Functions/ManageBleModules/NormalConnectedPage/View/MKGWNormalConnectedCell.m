...
@implementation MKGWNormalConnectedCellModel
...
@interface MKGWNormalConnectedCell ()
...
@implementation MKGWNormalConnectedCell

+ (MKGWNormalConnectedCell *)initCellWithTableView:(UITableView *)tableView {
    MKGWNormalConnectedCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGWNormalConnectedCellIdenty"];
    if (!cell) {
        cell = [[MKGWNormalConnectedCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGWNormalConnectedCellIdenty"];
    }
    return cell;
}
...
- (void)writeButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gw_normalConnectedCell_writeButtonPressed:row:serverUUID:characteristic:)]) {
        [self.delegate gw_normalConnectedCell_writeButtonPressed:self.dataModel.section
...
- (void)readButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gw_normalConnectedCell_readButtonPressed:row:serverUUID:characteristic:)]) {
        [self.delegate gw_normalConnectedCell_readButtonPressed:self.dataModel.section
                                                            row:self.dataModel.row
                                                     serverUUID:self.dataModel.server
                                                 characteristic:self.dataModel.characteristic];
    }
...
- (void)notifyButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gw_normalConnectedCell_notifyButtonPressed:section:row:serverUUID:characteristic:)]) {
        [self.delegate gw_normalConnectedCell_notifyButtonPressed:!self.notifyButton.selected
...
#pragma mark - setter
- (void)setDataModel:(MKGWNormalConnectedCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWNormalConnectedCellModel.class]) {
        return;
    }
    ...
    [self.notifyButton setImage:LOADICON(@"MKGatewayThree", @"MKGWNormalConnectedCell", @"gw_characteristic_notifyIcon.png") forState:UIControlStateNormal];
    self.notifyButton.selected = NO;
    if (_dataModel.notifyStatus == 1) {
        //打开了监听
        [self.notifyButton setImage:LOADICON(@"MKGatewayThree", @"MKGWNormalConnectedCell", @"gw_characteristic_receiveIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)writeButton {
    if (!_writeButton) {
        ...
        [_writeButton setImage:LOADICON(@"MKGatewayThree", @"MKGWNormalConnectedCell", @"gw_characteristic_writeIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)readButton {
    if (!_readButton) {
        ...
        [_readButton setImage:LOADICON(@"MKGatewayThree", @"MKGWNormalConnectedCell", @"gw_characteristic_readIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)notifyButton {
    if (!_notifyButton) {
        ...
        [_notifyButton setImage:LOADICON(@"MKGatewayThree", @"MKGWNormalConnectedCell", @"gw_characteristic_notifyIcon.png") forState:UIControlStateNormal];
