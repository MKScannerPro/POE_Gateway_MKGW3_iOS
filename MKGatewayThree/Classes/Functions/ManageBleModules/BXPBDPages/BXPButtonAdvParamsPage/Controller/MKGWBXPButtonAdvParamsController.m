//
//  MKGWBXPButtonAdvParamsController.m
...
@interface MKGWBXPButtonAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWBXPButtonAdvParamsModel *dataModel;

@end

@implementation MKGWBXPButtonAdvParamsController

- (void)dealloc {
    NSLog(@"MKGWBXPButtonAdvParamsController销毁");
}
...
- (void)gw_advNormalCell_setPressed:(NSInteger)index
    ...
    [MKGWMQTTInterface gw_bxpBtnConfigAdvParamsWithParams:param bleMac:self.bleMac macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (UITableViewCell *)loadTableCell:(NSIndexPath *)indexPath {
    ...
    if (!ValidDict(advParams)) {
        return [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGWBXPButtonAdvParamsControllerCell"];
    }
...
- (MKGWBXPButtonAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWBXPButtonAdvParamsModel alloc] init];
    }
    return _dataModel;
}
