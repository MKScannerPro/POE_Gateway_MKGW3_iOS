//
//  MKGWFilterByNanoBeaconController.m
...
@interface MKGWFilterByNanoBeaconController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWFilterByNanoBeaconModel *dataModel;

@end

@implementation MKGWFilterByNanoBeaconController

- (void)dealloc {
    NSLog(@"MKGWFilterByNanoBeaconController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWFilterByNanoBeaconController", @"gw_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKGWFilterByNanoBeaconModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWFilterByNanoBeaconModel alloc] init];
    }
    return _dataModel;
}
