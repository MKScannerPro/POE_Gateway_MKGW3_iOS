...
@interface MKGWAdvBeaconParamsModel : NSObject<gw_advertiseBeaconV2Protocol>
...
- (BOOL)configAdvBeaconDatas {
    ...
    if (self.isV2) {
        ...
        [MKGWMQTTInterface gw_configAdvertiseBeaconParams:dataModel macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
            success = YES;
            dispatch_semaphore_signal(self.semaphore);
        } failedBlock:^(NSError * _Nonnull error) {
            dispatch_semaphore_signal(self.semaphore);
...
- (void)operationFailedBlockWithMsg:(NSString *)msg block:(void (^)(NSError *error))block {
    moko_dispatch_main_safe(^{
        NSError *error = [[NSError alloc] initWithDomain:@"AdvBeaconModel"
                                                    code:-999
