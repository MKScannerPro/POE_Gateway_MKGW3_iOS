...
@implementation MKGWSDKDataAdopter

+ (NSString *)fetchWifiSecurity:(mk_gw_wifiSecurity)security {
    switch (security) {
        case mk_gw_wifiSecurity_personal:
            return @"00";
        case mk_gw_wifiSecurity_enterprise:
            return @"01";
    }
}
...
+ (NSString *)fetchWifiEapType:(mk_gw_eapType)eapType {
    switch (eapType) {
        case mk_gw_eapType_peap_mschapv2:
            return @"00";
        case mk_gw_eapType_ttls_mschapv2:
            return @"01";
        case mk_gw_eapType_tls:
            return @"02";
    }
}
...
+ (NSString *)fetchConnectModeString:(mk_gw_connectMode)mode {
    switch (mode) {
        case mk_gw_connectMode_TCP:
            return @"00";
        case mk_gw_connectMode_CASignedServerCertificate:
            return @"01";
        case mk_gw_connectMode_CACertificate:
            return @"02";
        case mk_gw_connectMode_SelfSignedCertificates:
            return @"03";
    }
}
...
+ (NSString *)fetchMqttServerQosMode:(mk_gw_mqttServerQosMode)mode {
    switch (mode) {
        case mk_gw_mqttQosLevelAtMostOnce:
            return @"00";
        case mk_gw_mqttQosLevelAtLeastOnce:
            return @"01";
        case mk_gw_mqttQosLevelExactlyOnce:
            return @"02";
    }
}
