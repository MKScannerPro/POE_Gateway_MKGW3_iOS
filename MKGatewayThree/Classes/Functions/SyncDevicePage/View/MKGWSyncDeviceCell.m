...
@implementation MKGWSyncDeviceCellModel
@end

@interface MKGWSyncDeviceCell ()
...
@implementation MKGWSyncDeviceCell

+ (MKGWSyncDeviceCell *)initCellWithTableView:(UITableView *)tableView {
    MKGWSyncDeviceCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKGWSyncDeviceCellIdenty"];
    if (!cell) {
        cell = [[MKGWSyncDeviceCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKGWSyncDeviceCellIdenty"];
    }
    return cell;
}
...
- (void)backBtnPressed {
    ...
    [self updateIcon];
    if ([self.delegate respondsToSelector:@selector(gw_syncDeviceCell_selected:index:)]) {
        [self.delegate gw_syncDeviceCell_selected:self.backBtn.selected index:self.dataModel.index];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKGWSyncDeviceCellModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWSyncDeviceCellModel.class]) {
        return;
    }
...
- (void)updateIcon {
    UIImage *btnIcon = (self.backBtn.selected ? LOADICON(@"MKGatewayThree", @"MKGWSyncDeviceCell", @"gw_listButtonSelectedIcon.png") : LOADICON(@"MKGatewayThree", @"MKGWSyncDeviceCell", @"gw_listButtonUnselectedIcon.png"));
