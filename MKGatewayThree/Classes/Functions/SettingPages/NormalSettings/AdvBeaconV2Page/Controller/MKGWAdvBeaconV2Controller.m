...
@interface MKGWAdvBeaconV2Controller ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWAdvBeaconV2Model *dataModel;

@end

@implementation MKGWAdvBeaconV2Controller

- (void)dealloc {
    NSLog(@"MKGWAdvBeaconV2Controller销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWAdvBeaconV2Controller", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWAdvBeaconV2Model *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWAdvBeaconV2Model alloc] init];
    }
    return _dataModel;
}
