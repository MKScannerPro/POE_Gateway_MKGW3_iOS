...
@implementation MKGWFilterRawAdvDataModel
...
@interface MKGWFilterByOtherModel ()
...
@implementation MKGWFilterByOtherModel
...
- (BOOL)configFilterByOther:(NSArray <NSDictionary *>*)list relationship:(mk_gw_filterByOther)relationship {
    NSMutableArray *tempList = [NSMutableArray array];
    for (NSDictionary *dic in list) {
        MKGWFilterRawAdvDataModel *model = [MKGWFilterRawAdvDataModel mk_modelWithJSON:dic];
        if (model) {
    ...
    __block BOOL success = NO;
    [MKGWMQTTInterface gw_configFilterByOtherDatas:self.isOn relationship:relationship rawDataList:tempList macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
