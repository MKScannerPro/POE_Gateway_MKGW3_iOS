//
//  MKGWBleNetworkSettingsController.m
...
@interface MKGWBleNetworkSettingsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWBleNetworkSettingsModel *dataModel;

@end

@implementation MKGWBleNetworkSettingsController

- (void)dealloc {
    NSLog(@"MKGWBleNetworkSettingsController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWBleNetworkSettingsController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWBleNetworkSettingsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWBleNetworkSettingsModel alloc] init];
    }
    return _dataModel;
}
