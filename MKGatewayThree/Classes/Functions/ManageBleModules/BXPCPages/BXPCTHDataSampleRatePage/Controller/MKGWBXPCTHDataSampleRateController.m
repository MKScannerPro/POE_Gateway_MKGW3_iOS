//
//  MKGWBXPCTHDataSampleRateController.m
...
@interface MKGWBXPCTHDataSampleRateController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWBXPCTHDataSampleRateModel *dataModel;

@end

@implementation MKGWBXPCTHDataSampleRateController

- (void)dealloc {
    NSLog(@"MKGWBXPCTHDataSampleRateController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWBXPCTHDataSampleRateController", @"gw_saveIcon.png") forState:UIControlStateNormal];
    self.titleLabel.font = MKFont(15.f);
...
- (MKGWBXPCTHDataSampleRateModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWBXPCTHDataSampleRateModel alloc] init];
    }
    return _dataModel;
}
