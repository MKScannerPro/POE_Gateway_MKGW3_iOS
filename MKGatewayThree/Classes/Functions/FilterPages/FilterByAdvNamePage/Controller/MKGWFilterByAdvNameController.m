//
//  MKGWFilterByAdvNameController.m
...
@interface MKGWFilterByAdvNameController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWFilterByAdvNameModel *dataModel;

@end

@implementation MKGWFilterByAdvNameController

- (void)dealloc {
    NSLog(@"MKGWFilterByAdvNameController销毁");
}
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        MKGWFilterEditSectionHeaderViewModel *headerModel = [[MKGWFilterEditSectionHeaderViewModel alloc] init];
        headerModel.index = 0;
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWFilterByAdvNameController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWFilterByAdvNameModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWFilterByAdvNameModel alloc] init];
    }
    return _dataModel;
}
