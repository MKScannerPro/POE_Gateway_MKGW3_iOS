...
- (BOOL)readTHDataSampleRate:(NSString *)bleMac {
    ...
    [MKGWMQTTInterface gw_readBXPSTHDataSampleRateWithBleMacAddress:bleMac macAddress:[MKGWDeviceModeManager shared].macAddress topic:[MKGWDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configTHDataSampleRate:(NSString *)bleMac {
    ...
    [MKGWMQTTInterface gw_configBXPSSampleRate:[self.self.sampleRate integerValue] bleMac:bleMac macAddress:[MKGWDeviceModeManager shared].macAddress topic:[MKGWDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
