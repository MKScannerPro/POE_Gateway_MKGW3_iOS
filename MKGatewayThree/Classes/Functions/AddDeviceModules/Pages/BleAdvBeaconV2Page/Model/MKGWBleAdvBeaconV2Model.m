...
@interface MKGWBleAdvBeaconV2Model ()
...
@implementation MKGWBleAdvBeaconV2Model
...
- (BOOL)readRssi1m {
    __block BOOL success = NO;
    [MKGWInterface gw_readBeaconRssiWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configRssi1m {
    __block BOOL success = NO;
    [MKGWInterface gw_configBeaconRssi:self.rssi1m sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readConnectable {
    __block BOOL success = NO;
    [MKGWInterface gw_readConnectableWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configConnectable {
    __block BOOL success = NO;
    [MKGWInterface gw_configConnectable:self.connectable sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
