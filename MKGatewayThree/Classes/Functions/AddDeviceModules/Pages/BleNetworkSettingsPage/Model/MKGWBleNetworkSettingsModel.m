...
@interface MKGWBleNetworkSettingsModel ()
...
@implementation MKGWBleNetworkSettingsModel
...
- (BOOL)configDHCPStatus {
    __block BOOL success = NO;
    [MKGWInterface gw_configWIFIDHCPStatus:self.dhcp sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configIpAddress {
    __block BOOL success = NO;
    [MKGWInterface gw_configWIFIIpAddress:self.ip
                                     mask:self.mask
