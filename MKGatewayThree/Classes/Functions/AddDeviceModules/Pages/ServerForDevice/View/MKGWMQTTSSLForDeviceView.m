...
@implementation MKGWMQTTSSLForDeviceViewModel
@end

@interface MKGWMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKGWMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(gw_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate gw_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(gw_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate gw_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate gw_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate gw_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(gw_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate gw_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKGWMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKGWMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKGatewayThree", @"MKGWMQTTSSLForDeviceView", @"gw_switchSelectedIcon.png") : LOADICON(@"MKGatewayThree", @"MKGWMQTTSSLForDeviceView", @"gw_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKGatewayThree", @"MKGWMQTTSSLForDeviceView", @"gw_switchUnselectedIcon.png") forState:UIControlStateNormal];
