//
//  MKGWServerConfigDeviceSettingView.m
...
@interface MKGWServerConfigDeviceSettingView ()
...
@implementation MKGWServerConfigDeviceSettingView
...
- (void)clearButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate gw_mqtt_deviecSetting_fileButtonPressed:2];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate gw_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(gw_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate gw_mqtt_deviecSetting_fileButtonPressed:1];
    }
}
