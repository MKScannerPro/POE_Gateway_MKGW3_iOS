...
@interface MKGWFilterByURLController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWFilterByURLModel *dataModel;

@end

@implementation MKGWFilterByURLController

- (void)dealloc {
    NSLog(@"MKGWFilterByURLController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKGatewayThree", @"MKGWFilterByURLController", @"gw_saveIcon.png") forState:UIControlStateNormal];
...
- (MKGWFilterByURLModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWFilterByURLModel alloc] init];
    }
    return _dataModel;
}
