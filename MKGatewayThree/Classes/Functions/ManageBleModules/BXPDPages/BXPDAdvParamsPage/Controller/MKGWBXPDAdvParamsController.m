...
@interface MKGWBXPDAdvParamsController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKGWDAdvParamsModel *dataModel;

@end

@implementation MKGWBXPDAdvParamsController

- (void)dealloc {
    NSLog(@"MKGWBXPDAdvParamsController销毁");
}
...
- (void)gw_BXPAdvParamsCell_setPressedWithSlotIndex:(NSInteger)slotIndex
    ...
    [MKGWMQTTInterface gw_configBXPDAdvParamsWithChannel:slotIndex interval:[interval integerValue] txPower:txPower bleMac:self.bleMac macAddress:[MKScannerDeviceModelManager shared].macAddress topic:[MKScannerDeviceModelManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
...
- (MKGWDAdvParamsModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKGWDAdvParamsModel alloc] init];
    }
    return _dataModel;
}
